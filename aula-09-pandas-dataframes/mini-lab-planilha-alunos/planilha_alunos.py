# =====================================================================
# Mini-lab: Construindo a Planilha de Alunos
# Criação · Missing Values · Filtros · groupby() · Exportação CSV
# =====================================================================
import pandas as pd
import numpy as np
...
df['idade'] = df.groupby('turma')['idade'].transform(
...
print('\nNaN restantes após limpeza:', df.isna().sum().sum())
print('\nVisualizando notas preenchidas:')
...
print('\nArquivos CSV salvos:')
print(' alunos_tratados.csv — dataset completo')
print(' resumo_turmas.csv   — resumo por turma')


# ── Desafios Extras ──────────────────────────────────────────
print("\n--- Processando Desafios Extras ---")

# Desafio 1: Adicionar nota_media_turma e delta_turma
...
# Desafio 2: Concatenar DataFrame simulando novos alunos
df_novos = pd.DataFrame({
    'id': [11], 'nome': ['Zeca'], 'turma': ['C'],
    'idade': [19], 'faltas': [0],
    'nota_1b': [10.0], 'nota_2b': [10.0], 'nota_3b': [10.0], 'nota_4b': [10.0]
})
# Processamento mínimo para preencher campos básicos da concatenação
df_novos['media_final'] = 10.0
df_expandido = pd.concat([df, df_novos], ignore_index=True)

# Desafio 3: Exportar para Excel com múltiplas abas usando ExcelWriter
with pd.ExcelWriter('alunos.xlsx') as writer:
    df.to_excel(writer, sheet_name='Alunos', index=False)
    resumo_turma.to_excel(writer, sheet_name='Resumo_Turmas', index=False)
    
print("Excel alunos.xlsx salvo com múltiplas abas (Alunos e Resumo_Turmas)!")
