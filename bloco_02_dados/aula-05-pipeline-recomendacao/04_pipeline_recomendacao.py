...
def recommend_products(seed_product_id, limit=3):
    ...
    cur.execute(
        ...
        SELECT
            ...
            COALESCE(AVG(ap.avaliacao), 0) AS media_avaliacoes,
...
if __name__ == "__main__":
    ...
    ingest_produtos_csv('01_produtos.csv')
    
    # 2. Ingestão de Avaliações
    ingest_avaliacoes_json('02_avaliacoes.json')
    ...
    if recs:
        for r_id, r_nome, r_categoria, r_distancia, r_media_aval, r_pos_aval in recs:
            media = float(r_media_aval) if r_media_aval is not None else 0.0
            pos = r_pos_aval if r_pos_aval is not None else 0
            print(f" - ID: {r_id}, Nome: {r_nome}, Categoria: {r_categoria}, Similaridade: {1 - r_distancia:.2f}, Média Aval.: {media:.1f}, Positivas: {pos}")
    else:
