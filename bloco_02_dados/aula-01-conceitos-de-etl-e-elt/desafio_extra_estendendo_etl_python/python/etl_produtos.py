"""
etl_produtos.py — ETL completo em Python puro (Baseline do Desafio Extra)
...
def extrair(caminho_csv: str) -> pd.DataFrame:
    ...
    if not os.path.exists(caminho_csv):
        # Tenta resolver o caminho a partir da raiz do desafio_extra
...
def carregar(validos: pd.DataFrame, rejeitados: pd.DataFrame) -> None:
    ...
    rejeitados.assign(pipeline_origem="etl_produtos_python").to_sql(
        "rejeitados", engine, schema="silver", if_exists="append",
        index=False
    )
    print(
...
if __name__ == "__main__":
    print("=== EXECUTANDO ETL PRODUTOS (BASELINE - PASSOS 1 E 2) ===")
