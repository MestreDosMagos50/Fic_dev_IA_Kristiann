...
ARQUIVO_CSV = BASE_DIR / "alunos.csv"
...
class NotaForaDoIntervaloError(DadosAlunoError):
    ...
    def __init__(self, campo: str, valor: float, linha: int):
        ...
        self.linha = linha
        super().__init__(
            f"Linha {linha}: '{campo}' = {valor} fora do intervalo [{NOTA_MIN}, {NOTA_MAX}]"
        )
...
def ler_alunos(caminho: str | Path) -> tuple[list[dict], list[dict]]:
...
def exibir_relatorio(validos: list[dict], erros: list[dict]) -> None:
    ...
    print(f" Registros válidos           : {len(validos)} ({len(validos)/total*100:.0f}%)")
    print(f" Registros com erro          : {len(erros)} ({len(erros)/total*100:.0f}%)")
